...
const Home = () => {
  ...
  // Thiết lập timer tự động chuyển bài
  useEffect(() => {
    timerRef.current = setInterval(goToNext, 5000); // 5000ms = 5 giây
    
    // Clear interval khi component unmount hoặc currentIndex thay đổi
    return () => {
      if (timerRef.current) {
  ...
  // Reset timer khi người dùng tương tác thủ công
  const resetTimer = () => {
    if (timerRef.current) {
      clearInterval(timerRef.current);
    }
  ...
  // Lấy 3 thẻ hiện tại: sub (trái), main, sub (phải)
